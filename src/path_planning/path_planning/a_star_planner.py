...
from custom_interfaces.srv import PlanPath  # <-- your custom srv
...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.map_sub = self.create_subscription(
            OccupancyGrid, "/inflated_costmap", self.map_callback, map_qos
        )
        ...
        # NEW: Service interface for other nodes (path_allocation)
        self.plan_service = self.create_service(
            PlanPath, "a_star/plan_path", self.plan_path_callback
        )
    ...
    # ---------------- RViz goal callback (old behavior) ----------------
    ...
    def goal_callback(self, pose: PoseStamped):
        ...
        try:
            map_to_base_tf = self.tf_buffer.lookup_transform(
                self.map_.header.frame_id, "tb3_1/base_footprint", rclpy.time.Time()
            )
    ...
    def plan_path_callback(self, request, response):
        """
        Service: given start & goal (in map frame), compute path & length.
        """
        if self.map_ is None:
        ...
        path = self.plan(request.start.pose, request.goal.pose)
        response.path = path

        if path.poses:
            response.length = float(self.compute_path_length(path))
            response.success = True
            response.message = "OK"
            self.get_logger().info("Service path planned successfully.")
        else:
            response.length = 0.0
            response.success = False
            response.message = "No path found"
            self.get_logger().warn("Service could not find a path.")
        return response
    ...
    def plan(self, start: Pose, goal: Pose):
        ...
        # Reconstruct path only if we reached goal
